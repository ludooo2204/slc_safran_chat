...
const ImportData = ({MAJdata})=>{
	const handleChange = (event) => {console.log(event.target.value);selectionCapteur(event.target.value) }
	const handleChangeTC = (event) => {console.log(event.target.value);setTypeTc(event.target.value) }
	...
	const getValue = () => {
		const handleVideData =(data) =>{
			...
			let dataTraitee=[reference,appareil,ecart,incertitudePlus,incertitudeMoins,incertitude];
			let dataToExport={
			nom:capteurSelectionne,
			reference:dataTraitee[0],
			appareil:dataTraitee[1],
			ecart:dataTraitee[2],
			incertitudePlus:dataTraitee[3],
			incertitudeMoins:dataTraitee[4],
			incertitude:dataTraitee[5],
			dateEtalonnage:new Date(dateValue).toLocaleDateString('FR-fr'),
			numeroCertificat:numCertificat,
			domaine:domaine,
	...
	console.log("dataToexport", dataToExport);
			...
			// console.log(regressionExponential(dataRegression,{order : 6,precision:12}))
			console.log(result);
		...
		// console.log(dataToExport);
		ajaxPost("post.php",JSON.stringify(dataToExport),function(rep){console.log(rep)},false)
	...
	<div className="nouvelEtalonnage ">		
		...
		<textarea id="textArea" autoFocus={true} placeholder="placer vos valeurs ici..." value={areaValue}  onChange={(event)=>setareaValue(event.target.value)} rows="20" cols="60"></textarea>
		<br/>
		<br/>
		<label>Date d'etalonnage</label>
		<input  type="date" value={dateValue} onChange={(event)=>setDateValue(event.target.value)} placeholder="date d'etalonnage jj/mm/aaaa"></input>
		<br/>
		<br/>
		...
		<div className="form-check" >
			<form>
			<input className="form-check-input" type="radio"  id="1"  value="SEC02C" checked={capteurSelectionne=="SEC02C"? true:false} onChange={handleChange}/>SEC02C
			<input className="form-check-input" type="radio"  id="2"  value="PRIM02" checked={capteurSelectionne=="PRIM02"? true:false} onChange={handleChange}/>PRIM02
			<input className="form-check-input" type="radio"  id="3"  value="PRIM03" checked={capteurSelectionne=="PRIM03"? true:false} onChange={handleChange}/>PRIM03
			<input className="form-check-input" type="radio"  id="4"  value="2257775−AMS" checked={capteurSelectionne=="2257775−AMS"? true:false} onChange={handleChange}/>2257775−AMS
			</form>
		</div>
{capteurSelectionne=="2257775−AMS"?(<div className="form-check" >
			<form>
			<input className="form-check-input" type="radio"  id="1"  value="TYPE K" checked={typeTc=="TYPE K"? true:false} onChange={handleChangeTC}/>TYPE K
			<input className="form-check-input" type="radio"  id="2"  value="TYPE N" checked={typeTc=="TYPE N"? true:false} onChange={handleChangeTC}/>TYPE N
			<input className="form-check-input" type="radio"  id="3"  value="TYPE S" checked={typeTc=="TYPE S"? true:false} onChange={handleChangeTC}/>TYPE S
			</form>
		</div>):<div></div>
