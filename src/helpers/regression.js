...
const regression = {
	...
	exponential(data, options) {
		...
		const predict = (x) => [round(x, options.precision), round(coeffA * Math.exp(coeffB * x), options.precision)];
	...
	logarithmic(data, options) {
		...
		const predict = (x) => [round(x, options.precision), round(round(coeffA + coeffB * Math.log(x), options.precision), options.precision)];
	...
	power(data, options) {
		...
		const predict = (x) => [round(x, options.precision), round(round(coeffA * x ** coeffB, options.precision), options.precision)];
